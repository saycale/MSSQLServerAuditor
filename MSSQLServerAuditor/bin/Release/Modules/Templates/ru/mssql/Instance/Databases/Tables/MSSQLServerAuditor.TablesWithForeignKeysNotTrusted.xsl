...
<root>
	<mssqlauditorpreprocessors name="Таблицы с ненадёжными внешними ключами" id="TablesWithForeignKeysNotTrusted.HTML.ru" columns="100" rows="100" splitter="yes">
		<mssqlauditorpreprocessor preprocessor="HtmlPreprocessorDialog" name="Таблицы с ненадёжными внешними ключами" id="TablesWithForeignKeysNotTrusted.HTML.ru" column="1" row="1" colspan="1" rowspan="1">
			...
			<body>
				...
				<xsl:if test="MSSQLResult[@name='GetTablesWithForeignKeysNotTrusted' and @SqlErrorNumber='0' and @hierarchy='']/RecordSet[@id='1']/child::node()">
				<table id="myTable">
				<thead>
					<tr>
						<th>
				...
				<tbody>
					<xsl:for-each select="MSSQLResult[@name='GetTablesWithForeignKeysNotTrusted' and @SqlErrorNumber='0' and @hierarchy='']/RecordSet[@id='1']/Row">
					<tr>
						<td>
							<xsl:choose>
								<xsl:when test="../../@instance != ''">
						...
						<td>
							<xsl:choose>
								<xsl:when test="ForeignKeyType != ''">
									<xsl:value-of select="ForeignKeyType"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
						...
						<td align="right">
							<xsl:choose>
								<xsl:when test="ForeignKeyCreateDate != ''">
									<xsl:value-of select="ms:format-date(ForeignKeyCreateDate, 'dd.MM.yyyy')"/>
									<xsl:text>&#160;</xsl:text>
									<xsl:value-of select="ms:format-time(ForeignKeyCreateDate, 'HH:mm:ss')"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
