...
<root>
	<mssqlauditorpreprocessors name="Связанные сервера" id="LinkedServers.HTML.ru" columns="100" rows="100" splitter="yes">
		<mssqlauditorpreprocessor preprocessor="HtmlPreprocessorDialog" name="Связанные сервера" id="LinkedServers.HTML.ru" column="1" row="1" colspan="1" rowspan="1">
			...
			<body>
				...
				<xsl:if test="MSSQLResult[@name='GetLinkedServers' and @SqlErrorNumber='0' and @hierarchy='']/RecordSet[@id='1']/child::node()">
				<table id="myTable">
				<thead>
					<tr>
						<th>
				...
				<tbody>
					<xsl:for-each select="MSSQLResult[@name='GetLinkedServers' and @SqlErrorNumber='0' and @hierarchy='']/RecordSet[@id='1']/Row">
					<tr>
						<td>
							<xsl:choose>
								<xsl:when test="SrvName != ''">
						...
						<td>
							<xsl:choose>
								<xsl:when test="SrvProduct != ''">
									<xsl:value-of select="SrvProduct"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
						...
						<td>
							<xsl:choose>
								<xsl:when test="SrvNetName != ''">
									<xsl:value-of select="SrvNetName"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
						...
						<td align="right">
							<xsl:choose>
								<xsl:when test="IsRemote != ''">
									<xsl:value-of select="IsRemote"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
						...
						<td align="right">
							<xsl:choose>
								<xsl:when test="RpcOut != ''">
									<xsl:value-of select="RpcOut"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
						...
						<td align="right">
							<xsl:if test="UsesSelfCredential != 'True'">
								<xsl:attribute name="style">font-weight: bold; color: red;</xsl:attribute>
							</xsl:if>
							<xsl:choose>
								<xsl:when test="UsesSelfCredential != ''">
									<xsl:value-of select="UsesSelfCredential"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
						...
						<td>
							<xsl:choose>
								<xsl:when test="LoginRemoteName != ''">
									<xsl:value-of select="LoginRemoteName"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
