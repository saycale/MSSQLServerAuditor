...
<root>
	<mssqlauditorpreprocessors name="Самые дорогостоящие процедуры по записи" id="InstanceMostExpensiveProceduresByWrites.HTML.ru" columns="100" rows="100" splitter="yes">
		<mssqlauditorpreprocessor preprocessor="HtmlPreprocessorDialog" name="Самые дорогостоящие процедуры по записи" id="InstanceMostExpensiveProceduresByWrites.HTML.ru" column="1" row="1" colspan="1" rowspan="1">
			...
			<body>
				...
				<xsl:if test="MSSQLResult[@name='GetInstanceMostExpensiveProceduresByWrites' and @SqlErrorNumber='0' and @hierarchy='']/RecordSet[@id='1']/child::node()">
				<table id="myTable">
				<thead>
					<tr>
						<th>
				...
				<tbody>
					<xsl:for-each select="MSSQLResult[@name='GetInstanceMostExpensiveProceduresByWrites' and @SqlErrorNumber='0' and @hierarchy='']/RecordSet[@id='1']/Row">
					<tr>
						<td>
							<xsl:choose>
								<xsl:when test="../../@instance != ''">
						...
						<td align="right">
							<xsl:choose>
								<xsl:when test="total_elapsed_time != ''">
									<xsl:value-of select="format-number(total_elapsed_time, '###,###,##0')"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
