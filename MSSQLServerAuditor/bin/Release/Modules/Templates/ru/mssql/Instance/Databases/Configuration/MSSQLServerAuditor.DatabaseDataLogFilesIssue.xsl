...
<root>
	<mssqlauditorpreprocessors name="Файлы данных и журнала транзакций на одном физическом диске" id="DatabaseDataLogFilesIssue.HTML.ru" columns="100" rows="100" splitter="yes">
		<mssqlauditorpreprocessor preprocessor="HtmlPreprocessorDialog" name="Файлы данных и журнала транзакций на одном физическом диске" id="DatabaseDataLogFilesIssue.HTML.ru" column="1" row="1" colspan="1" rowspan="1">
			...
			<body>
				...
				<xsl:if test="MSSQLResult[@name='GetDatabaseDataLogFilesIssue' and @SqlErrorNumber='0' and @hierarchy='']/RecordSet[@id='1']/child::node()">
				<table id="myTable">
				<thead>
					<tr>
						<th>
				...
				<tbody>
					<xsl:for-each select="MSSQLResult[@name='GetDatabaseDataLogFilesIssue' and @SqlErrorNumber='0' and @hierarchy='']/RecordSet[@id='1']/Row">
					<tr>
						<td>
							<xsl:choose>
								<xsl:when test="../../@instance != ''">
						...
						<td>
							<xsl:choose>
								<xsl:when test="DiskDrive != ''">
									<xsl:value-of select="DiskDrive"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
						...
						<td align="right">
							<xsl:choose>
								<xsl:when test="FileSizeMB != ''">
									<xsl:value-of select="format-number(FileSizeMB, '###,###,##0.00')"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
