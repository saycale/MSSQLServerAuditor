...
<root>
	<mssqlauditorpreprocessors name="Выполняемые хранимые процедуры" id="InstanceExecProcedures.HTML.ru" columns="100" rows="100" splitter="yes">
		<mssqlauditorpreprocessor preprocessor="HtmlPreprocessorDialog" name="Выполняемые хранимые процедуры" id="InstanceExecProcedures.HTML.ru" column="1" row="1" colspan="1" rowspan="1">
			...
			<body>
				...
				<xsl:if test="MSSQLResult[@name='GetInstanceProceduresExecution' and @SqlErrorNumber='0' and @hierarchy='']/RecordSet[@id='1']/child::node()">
				<table id="myTable">
				<thead>
					<tr>
						<th>
				...
				<tbody>
					<xsl:for-each select="MSSQLResult[@name='GetInstanceProceduresExecution' and @SqlErrorNumber='0' and @hierarchy='']/RecordSet[@id='1']/Row">
					<tr>
						<td align="right">
							<xsl:choose>
						...
						<td align="right">
							<xsl:choose>
								<xsl:when test="total_logical_reads != ''">
									<xsl:value-of select="format-number(total_worker_time, '###,###,##0')"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
