...
<root>
	<mssqlauditorpreprocessors name="DomainComputersByOperatingSystem" id="DomainComputersByOperatingSystem.HTML.en" columns="100" rows="100" splitter="yes">
		<mssqlauditorpreprocessor preprocessor="HtmlPreprocessorDialog" name="DomainComputersByOperatingSystem" id="DomainComputersByOperatingSystem.HTML.en" column="1" row="1" colspan="1" rowspan="1">
			...
			<body>
				...
				<xsl:if test="MSSQLResult[@name='GetListOfDomainComputersByOperatingSystem' and @SqlErrorNumber='0' and @hierarchy='']/RecordSet[@id='1']/child::node()">
				<table id="myTable">
				<thead>
					<tr>
						<th>
				...
				<tbody>
					<xsl:for-each select="MSSQLResult[@name='GetListOfDomainComputersByOperatingSystem' and @SqlErrorNumber='0' and @hierarchy='']/RecordSet[@id='1']/Row">
					<tr>
						<td>
							<xsl:choose>
								<xsl:when test="../../@instance != ''">
						...
						<td>
							<xsl:choose>
								<xsl:when test="LastLogonTimestamp != ''">
									<xsl:value-of select="LastLogonTimestamp"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
						...
						<td>
							<xsl:choose>
								<xsl:when test="DistinguishedName != ''">
						...
						<td>
							<xsl:choose>
								<xsl:when test="OperatingSystemServicePack != ''">
									<xsl:value-of select="OperatingSystemServicePack"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
						...
						<td>
							<xsl:choose>
								<xsl:when test="OperatingSystemVersion != ''">
