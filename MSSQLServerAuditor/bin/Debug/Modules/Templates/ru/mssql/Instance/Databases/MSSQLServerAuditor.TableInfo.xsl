...
<root>
	<mssqlauditorpreprocessors name="Описание объекта баз данных (информация)" id="TableInfo.HTML.ru" columns="100" rows="90;10" splitter="yes">
		<mssqlauditorpreprocessor preprocessor="HtmlPreprocessorDialog" name="Описание объекта баз данных (информация)" id="TableInfo.HTML.ru" column="1" row="1" colspan="1" rowspan="1">
			<xsl:stylesheet version="1.0"
				  xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
				  xmlns:ms="urn:schemas-microsoft-com:xslt"
				  xmlns:dt="urn:schemas-microsoft-com:datatypes">

			<xsl:template match="/MSSQLResults/MSSQLResult[not(@name='GetTableInfo' and @hierarchy='')]">
			</xsl:template>
			<xsl:template match="/MSSQLResults/MSSQLResult[@name='GetTableInfo' and @hierarchy='']">
			...
			<body>
				...
				<tbody>
					...
					<tr>
						...
						<td align="right">
							<xsl:choose>
								<xsl:when test="ObjectCreatedDate != ''">
									<xsl:value-of select="ms:format-date(ObjectCreatedDate, 'dd.MM.yyyy')"/>
									<xsl:text>&#160;</xsl:text>
									<xsl:value-of select="ms:format-time(ObjectCreatedDate, 'HH:mm:ss')"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
		...
		<mssqlauditorpreprocessor preprocessor="HtmlPreprocessorDialog" name="Результаты выполнения" id="TableInfo.HTML.Results.ru" column="1" row="2" colspan="1" rowspan="1">
			<xsl:stylesheet version="1.0"
				  xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
				  xmlns:ms="urn:schemas-microsoft-com:xslt"
				  xmlns:dt="urn:schemas-microsoft-com:datatypes">
