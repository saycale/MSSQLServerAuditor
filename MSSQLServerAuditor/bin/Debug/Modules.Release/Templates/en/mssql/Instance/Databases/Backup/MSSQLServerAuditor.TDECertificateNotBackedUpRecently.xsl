...
<root>
	<mssqlauditorpreprocessors name="TDE Certificate Not BackedUp Recently" id="TDECertificateNotBackedUpRecently.HTML.en" columns="100" rows="100" splitter="yes">
		<mssqlauditorpreprocessor preprocessor="HtmlPreprocessorDialog" name="TDE Certificate Not BackedUp Recently" id="TDECertificateNotBackedUpRecently.HTML.en" column="1" row="1" colspan="1" rowspan="1">
			...
			<body>
				...
				<xsl:if test="MSSQLResult[@name='GetTDECertificateNotBackedUpRecently' and @SqlErrorNumber='0' and @hierarchy='']/RecordSet[@id='1']/child::node()">
				<table id="myTable">
				<thead>
					<tr>
						<th>
						...
						<th>
							Latest Certificate Backup Date
						</th>
						<th>
				...
				<tbody>
					<xsl:for-each select="MSSQLResult[@name='GetTDECertificateNotBackedUpRecently' and @SqlErrorNumber='0' and @hierarchy='']/RecordSet[@id='1']/Row">
					<tr>
						<td>
							<xsl:choose>
								<xsl:when test="../../@instance != ''">
						...
						<td align="right">
							<xsl:choose>
								<xsl:when test="LatestCertificateBackupDate != ''">
									<xsl:value-of select="ms:format-date(LatestCertificateBackupDate, 'dd/MM/yyyy')"/>
									<xsl:text>&#160;</xsl:text>
									<xsl:value-of select="ms:format-time(LatestCertificateBackupDate, 'HH:mm:ss')"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
						...
						<td>
							<xsl:if test="CertificateBackupStatus != 0">
								<xsl:attribute name="style">font-weight: bold; color: blue;</xsl:attribute>
							</xsl:if>
							<xsl:choose>
								<xsl:when test="CertificateBackupStatus != ''">
									<xsl:value-of select="CertificateBackupStatus"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
