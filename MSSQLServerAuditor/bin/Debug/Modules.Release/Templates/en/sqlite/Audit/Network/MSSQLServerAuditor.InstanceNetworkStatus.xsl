...
<root>
	<mssqlauditorpreprocessors name="InstanceNetworkStatus" id="InstanceNetworkStatus.HTML.en" columns="100" rows="100" splitter="yes">
		<mssqlauditorpreprocessor preprocessor="HtmlPreprocessorDialog" name="InstanceNetworkStatus" id="InstanceNetworkStatus.HTML.en" column="1" row="1" colspan="1" rowspan="1">
			...
			<body>
				...
				<xsl:if test="MSSQLResult[@name='GetInstanceNetworkStatus' and @SqlErrorNumber='0' and @hierarchy='']/RecordSet[@id='1']/child::node()">
				<table id="myTable">
				<thead>
					<tr>
						<th>
				...
				<tbody>
					<xsl:for-each select="MSSQLResult[@name='GetInstanceNetworkStatus' and @SqlErrorNumber='0' and @hierarchy='']/RecordSet[@id='1']/Row">
					<tr>
						<td>
							<xsl:choose>
								<xsl:when test="../../@instance != ''">
						...
						<td>
							<xsl:choose>
								<xsl:when test="net_transport != ''">
									<xsl:value-of select="net_transport"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
						...
						<td align="right">
							<xsl:choose>
								<xsl:when test="num_reads != ''">
									<xsl:value-of select="format-number(num_reads, '###,##0')"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
						...
						<td align="right">
							<xsl:choose>
								<xsl:when test="num_writes != ''">
									<xsl:value-of select="format-number(num_writes, '###,##0')"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
						...
						<td align="right">
							<xsl:choose>
								<xsl:when test="bytes_reads != ''">
									<xsl:value-of select="format-number(bytes_reads, '###,##0')"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
						...
						<td align="right">
							<xsl:choose>
								<xsl:when test="bytes_writes != ''">
									<xsl:value-of select="format-number(bytes_writes, '###,##0')"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
