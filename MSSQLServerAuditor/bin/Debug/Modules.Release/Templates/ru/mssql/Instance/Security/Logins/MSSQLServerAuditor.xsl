...
<root>
	<mssqlauditorpreprocessors name="Пользователи сервера" id="ServerLogins.HTML.ru" columns="100" rows="100" splitter="yes">
		<mssqlauditorpreprocessor preprocessor="HtmlPreprocessorDialog" name="Пользователи сервера" id="ServerLogins.HTML.ru" column="1" row="1" colspan="1" rowspan="1">
			...
			<body>
				...
				<xsl:if test="MSSQLResult[@name='LoginsList' and @SqlErrorNumber='0' and @hierarchy='']/RecordSet[@id='1']/child::node()">
				<table id="myTable">
				<thead>
					<tr>
						<th>
				...
				<tbody>
					<xsl:for-each select="MSSQLResult[@name='LoginsList' and @SqlErrorNumber='0' and @hierarchy='']/RecordSet[@id='1']/Row">
					<tr>
						<td>
							<xsl:choose>
								<xsl:when test="../../@instance != ''">
						...
						<td align="right">
							<xsl:choose>
								<xsl:when test="CreateDateTime != ''">
									<xsl:value-of select="ms:format-date(CreateDateTime, 'dd.MM.yyyy')"/>
									<xsl:text>&#160;</xsl:text>
									<xsl:value-of select="ms:format-time(CreateDateTime, 'HH:mm:ss')"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
						...
						<td>
							<xsl:if test="IsMatchedDefaultLanguage != 1">
								<xsl:attribute name="style">font-weight: bold; color: red;</xsl:attribute>
							</xsl:if>
							<xsl:choose>
								<xsl:when test="LoginLanguage != ''">
									<xsl:value-of select="LoginLanguage"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
						...
						<td align="right">
							<xsl:if test="IsSysAdmin != 0">
								<xsl:attribute name="style">font-weight: bold; color: blue;</xsl:attribute>
							</xsl:if>
							<xsl:choose>
								<xsl:when test="IsSysAdmin != ''">
									<xsl:value-of select="IsSysAdmin"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
						...
						<td align="right">
							<xsl:if test="IsSecurityAdmin != 0">
								<xsl:attribute name="style">font-weight: bold; color: blue;</xsl:attribute>
							</xsl:if>
							<xsl:choose>
								<xsl:when test="IsSecurityAdmin != ''">
									<xsl:value-of select="IsSecurityAdmin"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
