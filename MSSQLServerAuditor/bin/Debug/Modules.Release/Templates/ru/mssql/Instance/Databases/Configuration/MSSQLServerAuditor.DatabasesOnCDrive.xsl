...
<root>
	<mssqlauditorpreprocessors name="Файлы база данных на диске C:\" id="DatabasesOnCDrive.HTML.ru" columns="100" rows="100" splitter="yes">
		<mssqlauditorpreprocessor preprocessor="HtmlPreprocessorDialog" name="Файлы база данных на диске C:\" id="DatabasesOnCDrive.HTML.ru" column="1" row="1" colspan="1" rowspan="1">
			...
			<body>
				...
				<xsl:if test="MSSQLResult[@name='GetDatabasesOnCDrive' and @SqlErrorNumber='0' and @hierarchy='']/RecordSet[@id='1']/child::node()">
				<table id="myTable">
				<thead>
					<tr>
						<th>
				...
				<tbody>
					<xsl:for-each select="MSSQLResult[@name='GetDatabasesOnCDrive' and @SqlErrorNumber='0' and @hierarchy='']/RecordSet[@id='1']/Row">
					<tr>
						<td>
							<xsl:choose>
								<xsl:when test="../../@instance != ''">
						...
						<td>
							<xsl:choose>
								<xsl:when test="DatabaseDiskDrive != ''">
									<xsl:value-of select="DatabaseDiskDrive"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
						...
						<td>
							<xsl:choose>
								<xsl:when test="DatabasePhysicalFileName != ''">
									<xsl:value-of select="DatabasePhysicalFileName"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
						...
						<td align="right">
							<xsl:choose>
								<xsl:when test="DatabasePhysicalFileState != ''">
									<xsl:value-of select="DatabasePhysicalFileState"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
						...
						<td align="right">
							<xsl:choose>
								<xsl:when test="DatabasePhysicalFileSize != ''">
									<xsl:value-of select="format-number(DatabasePhysicalFileSize, '###,###,##0.00')"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
