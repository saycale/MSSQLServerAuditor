...
<root>
	<mssqlauditorpreprocessors name="LogonHistory" id="LogonHistory.HTML.ru" columns="100" rows="100" splitter="yes">
		<mssqlauditorpreprocessor preprocessor="HtmlPreprocessorDialog" name="LogonHistory" id="LogonHistory.HTML.ru" column="1" row="1" colspan="1" rowspan="1">
			...
			<body>
				...
				<xsl:if test="MSSQLResult[@name='GetLogonHistory' and @SqlErrorNumber='0' and @hierarchy='']/RecordSet[@id='1']/child::node()">
				<table id="myTable">
				<thead>
					<tr>
						<th>
				...
				<tbody>
					<xsl:for-each select="MSSQLResult[@name='GetLogonHistory' and @SqlErrorNumber='0' and @hierarchy='']/RecordSet[@id='1']/Row">
					<tr>
						<td>
							<xsl:choose>
						...
						<td>
							<xsl:choose>
								<xsl:when test="ClientIPAddress != ''">
									<xsl:value-of select="ClientIPAddress"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
						...
						<td align="right">
							<xsl:choose>
								<xsl:when test="TimeGenerated != ''">
									<xsl:value-of select="ms:format-date(TimeGenerated, 'dd.MM.yyyy')"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
						...
						<td align="right">
							<xsl:choose>
								<xsl:when test="TimeGenerated != ''">
									<xsl:value-of select="ms:format-time(TimeGenerated, 'HH:mm:ss')"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
