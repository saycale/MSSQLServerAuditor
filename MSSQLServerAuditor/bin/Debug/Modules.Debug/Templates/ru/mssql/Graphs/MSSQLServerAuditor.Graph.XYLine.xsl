...
<root>
	<mssqlauditorpreprocessors name="Линейный график XY" id="XYLineGraph.HTML.ru" columns="100" rows="50;50" splitter="yes">
		<mssqlauditorpreprocessor preprocessor="GraphPreprocessorDialog" name="" id="XYLineGraph.Graph.ru" column="1" row="1" colspan="1" rowspan="1">
			<GraphConfiguration xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
				...
				<GraphSource xsi:type="XmlFileGraphSource">
					...
					<PathToItems>/MSSQLResults/MSSQLResult[@name='GetXYLineGraph' and @hierarchy='']/RecordSet[@id='1']</PathToItems>
					<ItemTag>Row</ItemTag>
					<NameTag>LineName</NameTag>
					<BaseTag>XPointValue</BaseTag >
					<ValueTag>YPointValue</ValueTag>
				</GraphSource>
		...
		<mssqlauditorpreprocessor preprocessor="HtmlPreprocessorDialog" name="" id="jXYLineGraph.Graph.ru" column="1" row="2" colspan="1" rowspan="1">
			<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
				...
				<xsl:template match="/MSSQLResults/MSSQLResult[@name='GetXYLineGraph' and @hierarchy='']/RecordSet[@id='1']">
						...
						<body>
							...
							<script type="text/javascript">
								$(document).ready(function()
								{
									...
									<xsl:for-each select="//Row[generate-id(.) = generate-id (key('LineName', LineName))]">
										<xsl:sort select="LineName"/>
										...
										<xsl:for-each select="key('LineName', LineName)">
											<xsl:value-of select="$var_name"/>.push(
												[
													<xsl:value-of select="XPointValue"/>,
													<xsl:value-of select="YPointValue"/>,
												]
											);
										</xsl:for-each>
