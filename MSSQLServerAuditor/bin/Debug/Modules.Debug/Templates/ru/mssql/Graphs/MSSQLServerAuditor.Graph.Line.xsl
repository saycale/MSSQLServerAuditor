...
<root>
	<mssqlauditorpreprocessors name="Линейный график" id="LineGraph.HTML.ru" columns="100" rows="50;50" splitter="yes">
		<mssqlauditorpreprocessor preprocessor="GraphPreprocessorDialog" name="" id="LineGraph.Graph.ru" column="1" row="1" colspan="1" rowspan="1">
			<GraphConfiguration xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
				...
				<GraphSource xsi:type="XmlFileGraphSource">
					...
					<PathToItems>/MSSQLResults/MSSQLResult[@name='GetLineGraph' and @hierarchy='']/RecordSet[@id='1']</PathToItems>
					<ItemTag>Row</ItemTag>
		...
		<mssqlauditorpreprocessor preprocessor="HtmlPreprocessorDialog" name="Линейный график jqPlot" id="jLineGraph.Graph.ru" column="1" row="2" colspan="1" rowspan="1">
			<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
				...
				<xsl:template match="/MSSQLResults/MSSQLResult[@name='GetLineGraph' and @hierarchy='']/RecordSet[@id='1']">
						...
						<body>
							...
							<script type="text/javascript">
								$(document).ready(function()
								{
									...
									<xsl:apply-templates select="//Row[PointValue1][1]/PointValue1" mode="plots" />
									<xsl:apply-templates select="//Row[PointValue2][1]/PointValue2" mode="plots" />
									<xsl:apply-templates select="//Row[PointValue3][1]/PointValue3" mode="plots" />
									<xsl:apply-templates select="//Row[PointValue4][1]/PointValue4" mode="plots" />
									...
									var plot = $.jqplot('chart', charts,
									{
										...
										series:
										[
											<xsl:apply-templates select="//Row[PointValue1][1]/PointValue1" mode="lines" />,
											<xsl:apply-templates select="//Row[PointValue2][1]/PointValue2" mode="lines" />,
											<xsl:apply-templates select="//Row[PointValue3][1]/PointValue3" mode="lines" />,
											<xsl:apply-templates select="//Row[PointValue4][1]/PointValue4" mode="lines" />
										]
									}
									);
				...
				<xsl:template match="//Row/PointValue1 | //Row/PointValue2 | //Row/PointValue3 | //Row/PointValue4" mode="plots">
				...
				<xsl:template match="//Row/PointValue1 | //Row/PointValue2 | //Row/PointValue3 | //Row/PointValue4" mode="lines">
					<xsl:variable name="LineName">
						<xsl:value-of select="local-name()" />
					</xsl:variable>

					<xsl:variable name="line_position" select="number(substring($LineName, 11))" />
					{
						label: '<xsl:value-of select="$LineName"/>',

						color: color_<xsl:choose>
							<xsl:when test="$line_position &lt;= 4">
								<xsl:value-of select="$line_position"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="4 - $line_position mod 4"/>
							</xsl:otherwise>
						</xsl:choose>
					}
				</xsl:template>
