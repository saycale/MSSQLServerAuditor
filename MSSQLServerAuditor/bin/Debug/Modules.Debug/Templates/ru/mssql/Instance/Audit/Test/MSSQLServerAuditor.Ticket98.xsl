...
<root>
	<mssqlauditorpreprocessors name="TestDataForTicket98" id="TestDataForTicket98.Graph.ru" columns="100" rows="50;50" splitter="yes">
		<mssqlauditorpreprocessor preprocessor="GraphPreprocessorDialog" name="" id="TestDataForTicket98.Graph.ru" column="1" row="1" colspan="1" rowspan="1">
			<GraphConfiguration xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
				...
				<GraphSource xsi:type="XmlFileGraphSource">
					...
					<PathToItems>/MSSQLResults/MSSQLResult[@name='GetTestDataForTicket98' and @hierarchy='']/RecordSet[@id='1']</PathToItems>
					<ItemTag>Row</ItemTag>
					<DateTag>ReportDateTime</DateTag>
					<NameTag></NameTag>
					<ValueTag>DataValue</ValueTag>
				</GraphSource>
		...
		<mssqlauditorpreprocessor preprocessor="HtmlPreprocessorDialog" name="" id="jTestDataForTicket98.Graph.ru" column="1" row="2" colspan="1" rowspan="1">
			<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
				...
				<xsl:template match="/MSSQLResults/MSSQLResult[@name='GetTestDataForTicket98' and @hierarchy='']/RecordSet[@id='1']">
						...
						<body>
							...
							<script type="text/javascript">
								$(document).ready(function()
								{
									...
									<xsl:apply-templates select="//Row[DataValue][1]/DataValue" mode="plots" />
									...
									var plot = $.jqplot('chart', charts,
									{
										...
										series:
										[
											<xsl:apply-templates select="//Row[DataValue][1]/DataValue" mode="lines" />
										]
									}
									);
				...
				<xsl:template match="//Row/DataValue" mode="plots">
						...
						<xsl:value-of select="$var_name"/>.push(
							[
								'<xsl:call-template name="formatDate">
									<xsl:with-param name="date" select="../ReportDateTime"/>
								</xsl:call-template>',
				...
				<xsl:template match="//Row/DataValue" mode="lines">
					<xsl:variable name="LineName">
						<xsl:value-of select="local-name()" />
					</xsl:variable>
					...
					<xsl:variable name="line_position" select="number(substring($LineName, 11))" />
					{
						...
						color: color_<xsl:choose>
							<xsl:when test="$LineName = 'DataValue'">
