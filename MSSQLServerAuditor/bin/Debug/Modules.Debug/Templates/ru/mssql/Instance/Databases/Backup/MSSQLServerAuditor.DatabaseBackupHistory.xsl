...
<root>
	<mssqlauditorpreprocessors name="История архивного копирование (таблицы)" id="DatabaseBackupHistory.HTML.ru" columns="100" rows="90;10" splitter="yes">
		<mssqlauditorpreprocessor preprocessor="HtmlPreprocessorDialog" name="История архивного копирование (таблицы)" id="DatabaseBackupHistory.HTML.ru" column="1" row="1" colspan="1" rowspan="1">
			<xsl:stylesheet version="1.0"
				  xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
				  xmlns:ms="urn:schemas-microsoft-com:xslt"
				  xmlns:dt="urn:schemas-microsoft-com:datatypes">

			<xsl:template match="/MSSQLResults/MSSQLResult[not(@name='GetDatabaseBackupHistory' and @hierarchy='')]">
			</xsl:template>
			<xsl:template match="/MSSQLResults/MSSQLResult[@name='GetDatabaseBackupHistory' and @hierarchy='']">
		...
		<mssqlauditorpreprocessor preprocessor="HtmlPreprocessorDialog" name="Результаты выполнения" id="DatabaseBackupHistory.HTML.Results.ru" column="1" row="2" colspan="1" rowspan="1">
			<xsl:stylesheet version="1.0"
				  xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
				  xmlns:ms="urn:schemas-microsoft-com:xslt"
				  xmlns:dt="urn:schemas-microsoft-com:datatypes">
	...
	<mssqlauditorpreprocessors name="Database Backup History (graph)" id="DatabaseBackupHistory.Graph.ru" columns="100" rows="50;50" splitter="yes">
		<mssqlauditorpreprocessor preprocessor="GraphPreprocessorDialog" name="" id="DatabaseBackupHistory.Graph.ru" column="1" row="1" colspan="1" rowspan="1">
			<GraphConfiguration xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
				...
				<GraphSource xsi:type="XmlFileGraphSource">
					...
					<PathToItems>/MSSQLResults/MSSQLResult[@name='GetDatabaseBackupHistory' and @hierarchy='']/RecordSet[@id='1']</PathToItems>
					<ItemTag>Row</ItemTag>
					<DateTag>BackupFinishDate</DateTag>
					<NameTag>DatabaseName</NameTag>
					<ValueTag>DatabaseBackupSizeMB</ValueTag>
		...
		<mssqlauditorpreprocessor preprocessor="HtmlPreprocessorDialog" name="" id="jDatabaseBackupHistory.Graph.ru" column="1" row="2" colspan="1" rowspan="1">
			<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
				<xsl:output method="html" encoding="utf-8" indent="yes" />

				<xsl:key name="DatabaseName" match="Row" use="DatabaseName" />
								...
								$(document).ready(function()
								{
									...
									<xsl:for-each select="//Row[generate-id(.) = generate-id (key('DatabaseName', DatabaseName))]">
										...
										<xsl:variable name="var_name">
											<xsl:value-of select="concat('chart_', DatabaseName)"/>
										</xsl:variable>
										...
										[
											...
											<xsl:sort select="DatabaseName"/>
											{
												...
												color: color_<xsl:choose>
													...
													<xsl:otherwise>
														<xsl:value-of select="4 - position() mod 4"/>
													</xsl:otherwise>
												</xsl:choose>
											}
