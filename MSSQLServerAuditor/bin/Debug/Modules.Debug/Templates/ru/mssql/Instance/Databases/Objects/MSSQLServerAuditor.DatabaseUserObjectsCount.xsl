...
<root>
	<mssqlauditorpreprocessors name="Список пользовательских объектов базы данных" id="DatabaseUserObjects.HTML.ru" columns="100" rows="90;10" splitter="yes">
		<mssqlauditorpreprocessor preprocessor="HtmlPreprocessorDialog" name="Список пользовательских объектов базы данных" id="DatabaseUserObjects.HTML.ru" column="1" row="1" colspan="1" rowspan="1">
			<xsl:stylesheet version="1.0"
				  xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
				  xmlns:ms="urn:schemas-microsoft-com:xslt"
				  xmlns:dt="urn:schemas-microsoft-com:datatypes">

			<xsl:template match="/MSSQLResults/MSSQLResult[not(@name='GetDatabaseUserObjectsCount' and @hierarchy='')]">
			</xsl:template>
			<xsl:template match="/MSSQLResults/MSSQLResult[@name='GetDatabaseUserObjectsCount' and @hierarchy='']">
			...
			<body>
				...
				<thead>
					<tr>
						...
						<th>
							Дата создания первого объекта
						</th>
						<th>
							Дата создания последнего объекта
						</th>
					</tr>
				</thead>
				<tbody>
					...
					<tr>
						...
						<td align="right">
							<xsl:choose>
								<xsl:when test="TypeCount != ''">
									<xsl:value-of select="format-number(TypeCount, '###,###,##0')"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
						...
						<td align="right">
							<xsl:choose>
								<xsl:when test="CreateDateMin != ''">
									<xsl:value-of select="ms:format-date(CreateDateMin, 'dd.MM.yyyy')"/>
									<xsl:text>&#160;</xsl:text>
									<xsl:value-of select="ms:format-time(CreateDateMin, 'HH:mm:ss')"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
						...
						<td align="right">
							<xsl:choose>
								<xsl:when test="CreateDateMax != ''">
									<xsl:value-of select="ms:format-date(CreateDateMax, 'dd.MM.yyyy')"/>
									<xsl:text>&#160;</xsl:text>
									<xsl:value-of select="ms:format-time(CreateDateMax, 'HH:mm:ss')"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>&#160;</xsl:text>
								</xsl:otherwise>
		...
		<mssqlauditorpreprocessor preprocessor="HtmlPreprocessorDialog" name="Результаты выполнения" id="DatabaseUserObjects.HTML.Results.ru" column="1" row="2" colspan="1" rowspan="1">
			<xsl:stylesheet version="1.0"
				  xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
				  xmlns:ms="urn:schemas-microsoft-com:xslt"
				  xmlns:dt="urn:schemas-microsoft-com:datatypes">
