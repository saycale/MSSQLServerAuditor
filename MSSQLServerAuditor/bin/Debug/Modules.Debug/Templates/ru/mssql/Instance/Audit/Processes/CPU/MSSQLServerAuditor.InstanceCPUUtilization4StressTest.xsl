...
<root>
	<mssqlauditorpreprocessors name="Стресс тесты: испльзование процессора" id="InstanceCPUUtilization4StressTest.Graph.ru" columns="100" rows="50;50" splitter="yes">
		<mssqlauditorpreprocessor preprocessor="GraphPreprocessorDialog" name="" id="InstanceCPUUtilization4StressTest.Graph.ru" column="1" row="1" colspan="1" rowspan="1">
			<GraphConfiguration xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
				...
				<GraphSource xsi:type="XmlFileGraphSource">
					...
					<PathToItems>/MSSQLResults/MSSQLResult[@name='GetInstanceCPUUtilization4StressTest' and @hierarchy='']/RecordSet[@id='1']</PathToItems>
					<ItemTag>Row</ItemTag>
		...
		<mssqlauditorpreprocessor preprocessor="HtmlPreprocessorDialog" name="" id="jInstanceCPUUtilization4StressTest.Graph.ru" column="1" row="2" colspan="1" rowspan="1">
			<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
				<xsl:output method="html" encoding="utf-8" indent="yes" />

				<xsl:key name="ReportId" match="Row" use="ReportId" />
				...
				<xsl:template match="/MSSQLResults/MSSQLResult[@name='GetInstanceCPUUtilization4StressTest' and @hierarchy='']/RecordSet[@id='1']">
						...
						<body>
							...
							<script type="text/javascript">
								$(document).ready(function()
								{
									...
									<xsl:for-each select="//Row[generate-id(.) = generate-id (key('ReportId', ReportId))]">
										<xsl:sort select="ReportId"/>

										<xsl:variable name="var_name">
											<xsl:value-of select="concat('chart_', ReportId)"/>
										</xsl:variable>

										var <xsl:value-of select="$var_name"/>=[];

										<xsl:for-each select="key('ReportId', ReportId)">
											<xsl:value-of select="$var_name"/>.push(
												[
													...
													<xsl:value-of select="OtherProcessesCPUUtilization"/>
												]
											);
										</xsl:for-each>
									...
									var plot = $.jqplot('chart', charts,
									{
										...
										[
											...
											<xsl:sort select="ReportId"/>
											{
												label: '<xsl:value-of select="ReportId"/>',
